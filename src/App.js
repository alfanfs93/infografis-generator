...
function ImagePreview({ prompt, rasioObj }) {
  const [status, setStatus] = useState("idle");
  const [imgSrc, setImgSrc] = useState("");

  const handleGenerate = () => {
    const encoded = encodeURIComponent(prompt.slice(0, 500));
    const url = `https://image.pollinations.ai/prompt/${encoded}?width=${rasioObj.w}&height=${rasioObj.h}&nologo=true&seed=${Math.floor(Math.random() * 99999)}`;
    setImgSrc(url);
    setStatus("loading");
  };
    ...
    <div style={{ background: "#1e2230", border: "1px solid #2a2f40", borderRadius: "12px", marginBottom: "14px", overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderBottom: "1px solid #2a2f40" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "8px", fontWeight: 600, fontSize: "13px" }}>
          ...
          Preview Gambar — Pollinations AI
        </div>
        <button
      ...
      <div style={{ padding: "16px", textAlign: "center" }}>
        {status === "idle" && (
          <div style={{ color: "#7a7f9a", fontSize: "13px", padding: "32px 0" }}>
            ...
            <div style={{ fontSize: "11px", marginTop: "6px", color: "#4a4f6a" }}>Kualitas preview — untuk final pakai Midjourney/Ideogram</div>
          </div>
        )}
        {status === "loading" && (
          <div style={{ color: "#7a7f9a", fontSize: "13px", padding: "32px 0" }}>
            <div style={{ fontSize: "28px", marginBottom: "8px" }}>⏳</div>
            Render gambar... (10–30 detik)
          </div>
        )}
        {imgSrc && (
          <img
            src={imgSrc} alt="Generated preview"
            style={{ maxWidth: "100%", maxHeight: "480px", borderRadius: "8px", display: status === "loading" ? "none" : "block", margin: "0 auto", border: "1px solid #2a2f40" }}
            onLoad={() => setStatus("loaded")}
            onError={() => setStatus("error")}
          />
        )}
        {status === "error" && (
          <div style={{ color: "#f87171", fontSize: "13px", padding: "16px 0" }}>
            Gagal load. Coba Regenerate atau cek koneksi internet.
          </div>
        )}
      </div>
    </div>
...
export default function App() {
  ...
  const [apiKeySaved, setApiKeySaved] = useState(!!localStorage.getItem("gemini_key"));
    ...
    <div style={c.page}>
      <div style={c.wrap}>
        ...
        <div style={{ ...c.card, borderColor: apiKeySaved ? "#1a3a2a" : "#2a2f40", background: apiKeySaved ? "#111a15" : "#161920" }}>
          <div style={c.sectionTitle}>🔑 Gemini API Key</div>
